...
- **その場で効く絞り込み。** セベリティのチェックボックス・日付/時刻範囲・無視パターン・一致パターンを、操作した瞬間に反映します。一致パターンは無視パターンのちょうど逆で、メッセージがマッチしたエントリ**だけ**を残します。WebviewではVS Code標準の `Ctrl+F` が使えないため、ハイライトして次/前へジャンプする検索ではなく「一致行のみ表示する」フィルタとして提供しています。どちらの欄も大文字小文字を無視する正規表現として解釈され（メタ文字を含まない文字列はそのまま部分一致として動作します）、一致パターンの判定対象はエントリのメッセージ部分のみです（タイムスタンプとセベリティは隣の絞り込みで既に扱えるため）。メッセージは継続行を含むので、スタックトレースのような複数行エントリはいずれかの行が一致すればエントリごと残ります。
