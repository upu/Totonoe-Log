...
`totonoeLog.highlightRules` にパターンを登録しておくと、Interactive View で一致した箇所に色が付きます。パネル側で切り替える操作はありません:
...
2つのルールの範囲が重なった場合は、先に書いたルールが優先されます（絞り込みたい範囲が狭いルールほど上に置いてください）。正規表現として不正なルールや、知らない色を指定したルールは、そのルールだけが名指しの警告付きで飛ばされ、残りのルールはそのまま効きます。設定を編集すると、開いているパネルの色がすぐ更新されます。
