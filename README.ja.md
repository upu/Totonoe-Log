...
絞り込みは「開き方」ではなく「開いたビューの表示状態」です。`Show Normalized View` や `Merge Selected Files` でビューを開いたあと、そのタブに対して `Totonoe Log: Set Filter`（エディタの右クリックメニューにもあります）を実行すると、複数の絞り込み条件を1つのフローで自由に組み合わせられます。複数選択リストから使いたい条件を選ぶと、選んだ条件のプロンプトだけが順に表示されます。
...
| 設定 | 型 | 既定値 | 説明 |
| --- | --- | --- | --- |
| `totonoeLog.gap.thresholdSeconds` | number | `30` | `Show Normalized View` と `Merge Selected Files` のビューで、連続するエントリのタイムスタンプ差がこの秒数以上のときに「XX秒の空白」の区切り行を挿入する。`Set Filter` で絞り込んだ後の並びにも適用される。`0` で無効化。 |
