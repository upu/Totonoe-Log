...
`Totonoe Log: Show Interactive View (Alpha)` は、正規化したログをWebviewパネルで開き、セベリティ・日付範囲・一致パターン・無視パターンの絞り込みをQuickPickダイアログの往復なしにその場でトグルできます。一致パターンは無視パターンのちょうど逆で、メッセージがマッチしたエントリ**だけ**を残します。WebviewではVSCode標準のCtrl+Fが使えないため、ハイライトして次/前へジャンプする検索ではなく「一致行のみ表示する」フィルタとして提供しています。どちらの欄も大文字小文字を無視する正規表現として解釈されます（メタ文字を含まない文字列はそのまま部分一致として動作します）。一致パターンの判定対象はエントリのメッセージ部分のみです（タイムスタンプとセベリティは隣の絞り込みで既に扱えるため）。メッセージは継続行を含むので、スタックトレースのような複数行エントリはいずれかの行が一致すればエントリごと残ります。エクスプローラでログファイルを右クリックして同じコマンドを選べば、そこからも開けます（複数選択していればまとめて開きます）。パネル内の「+ Add Files...」ボタンからは、同じパネルに別のログファイルを追加読み込みでき、2ファイル以上になった時点でファイル名/種別列付きのマージ表示に切り替わります（エクスプローラの選択は複数フォルダにまたがれますが、「+ Add Files...」のダイアログはまたげません）。読み込み済みファイルは1件ずつ並び、チェックボックスでそのファイルの行だけを表示/非表示にでき（絞り込みの一軸なので上記の絞り込みと組み合わせて効き、「Export as Virtual Document」にも反映されます）、隣の「✕」で読み込み自体を取り消せます。ファイルを非表示にしても、そのファイルのセベリティは絞り込みパネルに残り「N / M 行を表示」の分母も変わりません（セベリティのチェックを外したときと同じ扱いです）。最後の1ファイルは取り消せないので、一時的に消したい場合はチェックを外してください。行をダブルクリックするか、右クリックメニューから「Totonoe Log: Go to Source Line」を選ぶと元ファイルの該当行へジャンプし、ホバーすると元ファイルのフルパスと行番号を表示します（シングルクリックではジャンプしないため本文の選択を邪魔しません。ギャップマーカー等の生成行には対応する元行が無いためジャンプ対象外で、折りたたみグループの見出し行は従来どおりクリックで展開/復元します）。結果が大きい場合は `totonoeLog.interactiveView.maxDisplayLines` 行までしか描画せず、絞り込むか、パネルの「Export as Virtual Document」ボタンで全体を開くよう案内します。マスクボタン（「🔓 Mask」/「🔒 Mask」）を押すと、選んだ対象をプレースホルダーに置き換えて表示し、issueやチャットにそのまま貼れる状態にできます。隣の「▾」で伏せる対象——タイムスタンプ・ホスト名/IPアドレス・プロセスID・任意パターン——を選べます。プロセスIDは、syslog形式の `sshd[1234]:` と `pid=1234` / `pid: 1234` / `[pid 1234]` のようにpidと明記された表記を `<PID>` に置き換えます（log4jのスレッド名 `[main]` や配列の添字 `retries[3]` は誤マスクしません）。ユーザー名・トークン・契約IDなど、汎用ルールでは拾えない社内固有の識別子には2つの欄があります。まず使うのは「キー」欄です——伏せたいキー名をカンマまたは空白区切りで並べる（例: `user, token`）と、その**値だけ**が `<MASKED>` に置き換わるので、`user=hoge` は `user=<MASKED>` になりキー名は読めるまま残ります。`key=value` / `key: value` とクォート付きの値（`token="abc"` → `token="<MASKED>"`）に対応し、キー名は大文字小文字を無視してリテラルとして扱うため、正規表現のメタ文字や非ASCIIのキー名（`契約ID`）もそのまま書けます（`user` の指定で `superuser=x` のような別のキーは誤マスクしません）。「任意パターン」欄は正規表現の一致箇所を全て `<MASKED>` に置き換える欄で、キー欄では表せない対象に使います。不正な正規表現や処理が重いパターンでも、効かなくなるのはそのマスクだけで、警告を出したうえで他のマスクは効き続けます。2つの欄は同時に効き、どちらもパネルの状態として持ち設定には保存しません。ワンショットのコピー操作ではなく表示状態なので、通常どおりコピー（Ctrl+C）・検索ができ、「Export as Virtual Document」もマスク済みで書き出されます。既定はOFFで、マスク対象の初期選択は `Totonoe Log: Copy Masked Text` と共通の `totonoeLog.copyMasked.*` 設定に従います。下記の `totonoeLog.*` 設定を変更すると、パネルを開いたまま即座に表示へ反映します（パース結果が変わる `timestampFormats`・`timezone.sourceOffset`・`timezone.fileOffsets`・`clockSkew.fileOffsets` は読み込み済みファイルの再パースまで行い、それ以外は現在のエントリを描き直します）。Webviewベースのインタラクティブビューを試す実験的な最初の一歩です（issue #165/#166/#168）。挙動は今後変わる可能性があり、上記の読み取り専用の仮想ドキュメントビュー群が引き続き主な・安定した利用手段です。
