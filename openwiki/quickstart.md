...
現在の `package.json` と変更履歴は v0.10.0 を示す。このリリースでは、機能ごとのコマンドを増やすのではなく、対象数を自動判定する `totonoeLog.openVirtualDocument` と、開いているビューの状態を変える `totonoeLog.setViewFilter` へ操作を統合した。
...
v0.9.0前後ではInteractive Viewを中心に、複数パターン、ハイライト編集、マージ後の折りたたみ、認識率警告、マスク後の見た目に基づく折りたたみが追加された。v0.10.0ではマルチルート設定の修正、フィルタと仮想文書コマンドの統合、正規表現ワーカーのタイムアウト後処理に加え、セベリティ列と複数行エントリの継続行をメッセージ列へ整列した。これは「調査操作を1か所へ集約しつつ、非同期処理と表示の正確性を強化する」流れと読める。
