...
- **タイムスタンプ認識率が低いときの警告から、その補助へ直接飛べるように
  した。** モーダル通知・Interactive View のパネル警告行のどちらにも
  「タイムスタンプ形式の補助を開く」ボタンが付き、押すと対象ファイルで
  Interactive View を開き（または前面に出し）、タイムスタンプ形式パネルが
  展開された状態になる（issue #321）。
