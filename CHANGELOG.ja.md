...
- `Totonoe Log: Show Interactive View (Alpha)` で、表示中のログをマスクできる
  ようにした。マスクボタン（「🔓 マスク: OFF」/「🔒 マスク: ON」）でタイムスタンプ・ホスト名/IPアドレスを
  プレースホルダーに置き換えて表示し、隣の「▾」で2種のどちらを伏せるかを
  選べる（issue #180・#194、#165 の次のアルファ版フェーズ）。ワンショットの
...
- `Totonoe Log: Show Interactive View (Alpha)` をエクスプローラから直接開ける
  ようにした。ログファイルを2つ以上選んで右クリックし、このコマンドを選ぶと、
  マージ表示の1パネルとしてまとめて開ける（issue #181、#165 の次のアルファ版
